...
export default function BoardPage() {
  ...
  const [searchTerm, setSearchTerm] = useState(""); // ← 🔍 nova funcionalidade
  ...
  return (
    <main className="min-h-screen bg-blue-100 p-8">
      ...
      <h1 className="text-3xl font-bold text-blue-900 mb-4">{board.title}</h1>

      {/* 🔍 Campo de busca de colunas */}
      <div className="mb-6 max-w-md">
        <input
          ...
          className="w-full px-4 py-2 border border-blue-300 rounded text-gray-800 placeholder-gray-500 shadow-sm"
        />
      </div>
      ...
      {board.columns.length === 0 ? (
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 max-w-full mx-auto">
          {board.columns
            .filter((column) =>
            ...
            .map((column) => (
              <div
                key={column.id}
                className="bg-white p-4 rounded shadow w-64 flex-shrink-0 border border-blue-200 hover:shadow-md transition"
              ...
              >
                ...
                <ul className="space-y-2 mb-4">
                  {column.cards.map((card) => (
                    <li
                      key={card.id}
                      className="bg-gray-100 rounded p-2 text-gray-800 shadow-sm cursor-move hover:bg-gray-200 transition"
                      draggable
                      onDragStart={(e) => {
